...
const TradeSetupCard = ({ prediction, dataSource }: TradeSetupCardProps) => {
  ...
  const riskRewardRatio = prediction.profitLoss 
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-primary/10 border border-primary/20 text-center">
                ...
                <div className="text-xl font-bold text-primary">
                  ₹{prediction.premium.buyLeg.toFixed(2)}
                </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20">
                ...
                <div className="text-2xl font-bold text-green-500">
                  +₹{Math.abs(prediction.profitLoss.target).toLocaleString('en-IN')}
                </div>
                <div className="text-xs text-green-500">
                  +{prediction.expectedReturn?.toFixed(1) || 40}% return
                </div>
              </div>
              <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
                <div className="text-xs text-muted-foreground mb-1">At Stop Loss</div>
                <div className="text-2xl font-bold text-red-500">
                  -₹{Math.abs(prediction.profitLoss.stopLoss).toLocaleString('en-IN')}
                </div>
            ...
            <div className="flex justify-between items-center p-3 rounded-lg bg-muted/30 border border-border">
              <div>
                ...
                <div className="font-semibold">₹{prediction.profitLoss.breakeven.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</div>
              </div>
